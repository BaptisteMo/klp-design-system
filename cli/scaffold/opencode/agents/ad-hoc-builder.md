---
description: |
  Stage 2 of /klp-design. Builds reusable custom components under
  src/components/custom/ and stages inline JSX snippets for one-offs.
  Only runs when plan.ad_hoc is non-empty.
mode: subagent
temperature: 0.2
forge-level: project
forge-role: producer
...
3. **For each `reuse: "custom"` entry:**
   ...
   - Follow DS conventions: function component, `cn()` from `@klp/lib/cn`, `class-variance-authority` only if there are real variant axes, Tailwind utilities on token aliases only. No hex, no inline SVG, no `@radix-ui/*` imports.
...
4. **For each `reuse: "inline"` entry:**
   ...
   - Snippets may call DS components (`@klp/components/*`) freely.
...
- Never touch DS components at `external/klp-design-system/src/components/`.
