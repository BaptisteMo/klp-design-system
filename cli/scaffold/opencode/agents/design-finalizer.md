---
description: |
  Stage 4 of /klp-design. Appends gap entries + design-log lines,
  (re)creates src/mockups/_index.tsx if missing, moves the YAML from
  pending to to-be-review, cleans staging.
mode: subagent
temperature: 0.2
forge-level: project
forge-role: reviewer
...
4. **If `src/mockups/_index.tsx` does not exist**, copy the shipped seed from the manifest scaffold (it was already placed by `klp-ui init` or `klep-ds-init`; if truly missing, print a warning and skip — the user can re-run init or create it by hand).
