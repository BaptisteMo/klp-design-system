---
description: |
  Stage 3 of /klp-design. Writes one React file per screen under
  src/mockups/<id>/ and a _meta.json per request. Uses DS components +
  ad-hoc components from Stage 2.
mode: subagent
temperature: 0.2
forge-level: project
forge-role: producer
...
3. **For each screen in order**, write `src/mockups/<id>/<screen>.tsx`:
   ...
   - Import DS components as `import { X } from '@klp/components/<name>'`.
   - Import ad-hoc reusables as `import { X } from '@/components/custom/<name>'`.
   - Inline snippets must be inlined into the screen file (copy their JSX content into the screen). Do NOT import from `.klp/staging/...` — the staging dir is deleted by Stage 4.
...
- If a DS component referenced in `plan.json` is not present under `external/klp-design-system/src/components/`, abort with a clear message — the consumer can install it with `klp-ui add <name>`.
