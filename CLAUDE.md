...
- Bootstrap a new component: `/klp-build-component <Figma node name>` — orchestrates 4 subagents in sequence:
  1. `figma-extractor` — captures the Figma spec (`.klp/figma-refs/<name>/spec.json`) with per-layer token bindings
  2. `component-adapter` — writes the React source + playground route + registry stub
  3. `visual-verifier` — Playwright screenshot diff against Figma references (max 3 correction passes)
  4. `documentalist` — generates `docs/components/_index_<name>.md` + updates `klp-components.json` + runs the cross-reference reverse-index pass (non-blocking; failure here doesn't abort the commit gate)
...
- Don't commit files inside `.klp/verify-reports/` or `docs/.lint-report.md` — these are local diagnostics.
...
- Agent definitions: `.claude/agents/{figma-extractor,component-adapter,visual-verifier,documentalist}.md`
