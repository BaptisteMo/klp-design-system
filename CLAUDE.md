...
- Every component carries an entry in `.klp/intent.yaml` (`whenToUse` + `whenNotToUse` required, plus `aliases` / `figmaName` / `family` / `confusedWith` where they apply). A new component is not done until its entry exists — `node scripts/validate-intent.mjs` fails otherwise.
...
- Don't commit Figma reference screenshots. `.klp/figma-refs/**/*.png` is gitignored: the PNGs are an authoring aid for building a component against its Figma source, never needed to consume the DS. They stay on disk for visual review — `spec.json` remains tracked, since the token validator and the documentalist read it.
- Don't commit local agent tooling or scratch work. The repo is a distribution surface: it carries components plus the minimum needed to run the DS in an external project, nothing else. `.agents/`, `plans/`, `skills-lock.json`, third-party skills under `.claude/skills/` and failed Figma captures (`*.FAILED`) are all gitignored — never force-add them. `.claude/agents/`, `.claude/commands/` and the `klp-*` skills stay tracked: they are the pipeline.
...
- Don't edit the `KLP:INTENT` block on a doc page, or the `intent` / `aliases` / `exports` / `family` / `figmaName` fields in `klp-components.json` — regenerated by `pnpm run sync:intent` from `.klp/intent.yaml`. Edit the YAML.
...
- Component intent (hand-authored): `.klp/intent.yaml` — validator `scripts/validate-intent.mjs`, merger `scripts/merge-intent.mjs`, doc injector `scripts/inject-doc-intent.mjs`
- Token vocabulary: `docs/tokens/vocabulary.md` (generated by `pnpm run sync:tokens`)
...
- `docs/agent-brief.md` is generated by `scripts/build-agent-brief.ts`. Run `pnpm run build:agent-brief` after any change to `klp-components.json` or `src/styles/tokens/aliases.css`. `pnpm run build:all` runs the full chain (`sync:tokens` → `sync:intent` → `build:agent-brief` → `build:manifest` → `validate:manifest`).
