...
- [Table Row](../components/_index_table-row.md) — `h-klp-size-3xl` (row height, non-empty); `pt-klp-size-3xl pr-klp-size-3xl pb-klp-size-3xl pl-klp-size-3xl` (empty root padding); `gap-klp-size-2xl` (empty root item spacing); `gap-klp-size-xs`, `pt-klp-size-xs pr-klp-size-xs pb-klp-size-xs pl-klp-size-xs` (cell-actions and action-button padding); `gap-klp-size-2xs` (cell-badge item spacing).
